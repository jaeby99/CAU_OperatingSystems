...
static void parse_input(struct vehicle_info *vehicle_info, char *input)					// parse input into 3 char (aAB) and cut by ':'
{
	int cnt;
	char *token, *save_ptr;
...
static int is_finished(struct vehicle_info *vehicle_info, int thread_cnt)		//도로위에 남은 차량
{
	int i, res;
...
void run_crossroads(char **argv)
{
	...
	thread_cnt = 1;
	for (i=0; (size_t) i<strlen(argv[1]); i++) {			// 차량 갯수 카운트+1
		if (argv[1][i] == ':') {
			thread_cnt++;
		}
	}
	...
	parse_input(vehicle_info, argv[1]);										// 각각의 차량 정보입력 
